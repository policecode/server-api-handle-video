...
async function changePassword_1(browser, filePath, config) {
    try {
        ...
        let process = true;
        let page;
        page = await newTab(browser, 'https://myaccount.google.com/signinoptions/password');
        if (!page) {
            console.log('error tại vị trí open newTab google');
            return false;
        }
        ...
        await pressKey(page, newPassword);
        // confirm pass mới

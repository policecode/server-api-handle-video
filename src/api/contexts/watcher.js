...
const { clickSelectorElement } = require('../../helpers/everything.helper');
...
module.exports = class {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (!this.#parent.videoInfo.isLive) {
          await Promise.race([
            this.#page.waitForSelector(`#segmented-like-button`, { timeout: 15000 }),
        ...
        try {
          await this.#page.exposeFunction('videoStateChanged', videoStateChanged);
        } catch (e) {
          LOGGER.warn(`Page container: ${e} `);
        }
  ...
  async like() {
    return new Promise(async (resolve, reject) => {
      ...
      let em = await Promise.race([
        this.#page.waitForSelector(`ytd-segmented-like-dislike-button-renderer:nth-child(1) > div:nth-child(1) > ytd-toggle-button-renderer:nth-child(1) > yt-button-shape:nth-child(1) > button:nth-child(1)`, { timeout: 20000 }),
        this.#page.waitForSelector(`#segmented-like-button > ytd-toggle-button-renderer > yt-button-shape > button > yt-touch-feedback-shape > div > div.yt-spec-touch-feedback-shape__fill`, { timeout: 20000 }),
      ...
      try {
        let arias = await this.#page.$$eval('#segmented-like-button > ytd-toggle-button-renderer > yt-button-shape > button', as => as.map(a => a.getAttribute('aria-pressed')));
      ...
      if (em) {
        await clickSelectorElement(this.#page, em).catch(reject);
  ...
  async subscribe() {
    return new Promise(async (resolve, reject) => {
      let em = await Promise.race([
        this.#page.waitForSelector(`#subscribe-button > ytd-subscribe-button-renderer > yt-button-shape > button > yt-touch-feedback-shape > div > div.yt-spec-touch-feedback-shape__fill`, { timeout: 10000 }),
        this.#page.waitForSelector(`#subscribe-button > ytd-subscribe-button-renderer > yt-smartimation > yt-button-shape > button > yt-touch-feedback-shape > div > div.yt-spec-touch-feedback-shape__fill`, { timeout: 10000 }),
        this.#page.waitForSelector(`yt-button-shape > button > yt-touch-feedback-shape > div > div.yt-spec-touch-feedback-shape__fill`, { timeout: 10000 }),
        this.#page.waitForSelector(`ytd-reel-player-overlay-renderer:nth-child(1) > div:nth-child(2) > div:nth-child(3) > ytd-like-button-renderer:nth-child(1) > ytd-toggle-button-renderer:nth-child(1) > yt-button-shape:nth-child(1) > label:nth-child(1)`, { timeout: 10000 })
      ]).catch(reject);
